...
The *atlas* service is generated by the job *create-resources*. This job contains all the logic to create an Atlas cluster. Behind the hood the cluster's generation process is done in a container built with this [Dockerfile](./Dockerfile). This container calls [render.sh](./scripts/render.sh) to create the Atlas cluster and a database user. 

The *username* and *password* secrets are used to generated username / password on the fly, these ones are provided to the job via environment variable so it can create db user.

The *user* secret is generated by the *create-resources* job, it contains the credentials of the db user.
...
```
VERSION=v...
acorn build -t ghcr/lucj/acorn-mongo-atlas:$VERSION -t ghcr.io/lucj/acorn-mongo-atlas:latest .
```

Next we push the image to an OCI registry (GitHub Container Registry in this example):

```
acorn push ghcr.io/lucj/acorn-mongo-atlas:$VERSION
acorn push ghcr.io/lucj/acorn-mongo-atlas:latest
```
